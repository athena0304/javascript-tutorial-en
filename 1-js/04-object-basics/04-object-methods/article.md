...
说实话，这种表示法还是有些不同。有一些细微的差别与对象继承有关（稍后将会介绍），但现在它们无关紧要。在几乎所有的情况下，较短的语法是首选。
...
在 `user.sayHi()` 执行过程中，`this` 的值是 `user`。
...
在运行时对 `this` 求值的这个想法有其优缺点。一方面，函数可以被重用于不同的对象。另一方面，更大的灵活性给错误留下了空间。
